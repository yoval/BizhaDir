					  ...
					  <tbody>
					    ...
					    <tr id = "id<?php echo $i; ?>">
						    ...
						    <td class = "layui-hide-xs">
							    <?php if($fsize != '-'){ ?>
								<a href="javascript:;" class = "btn btn-sm btn-outline-primary" onclick = "scopy('<?php echo $name ?>')">复制</a>
							    <?php } ?>
							    <!--如果是管理模式-->
							    <?php if((isset($admin)) && ($fsize != '-')) { ?>
									<a href="javascript:;" class = "btn btn-sm btn-outline-primary btn-danger" onclick = "delfile(<?php echo $i; ?>,'<?php echo $name; ?>','<?php echo $fullpath; ?>')">删除</a>
							    <?php } ?>
							    <!--如果是markdown文件-->
